...
Several optionsa are available in the admin settings page which control how the synchronisation works:

SYNCHRONISE NOW		 			- on its own, it simply ensues all active Drupal accounts exist on PHPlist, and syncs any defined attributes

UPDATE DEFAULT SUBSCRIPTIONS 	- checking this box (re-)applies all default auto-subscribe options to all users.  This allows you to mass subscribe all your 
								users on a new install, for exmaple, but shouldn't really be used on an active system as it potentially overwrites any changes
								your users have made to their own subscriptions.
								
REMOVE EXISTING SUBSCRIPTIONS	- this does what it says.  It can only be used with the previous option, and it simply unsubscribes each user from EVERY list
								before re-applying the defaults.  Could be useful if you change which roles can see which lists, as users are not automatically
								unsubscribed from lists that they no longer have access to.  Again, use with caution.
								
Whenever an individual user's roles are updated, their subscriptions are checked to ensure that they are only subscribed to lists that their current role(s)
give them access.
...
account holders as Drupal has handled that side of things).  The url 'newsletters/confirm' allows your anonymous user to activate their
...
the email address field will be automatically populated.  This mechanism doesn't allow unsubscription from a particular newsletter - it's an all or nothing choice!
